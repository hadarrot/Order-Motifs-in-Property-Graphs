...
START_LEVEL = -100  # Crucial: Bitcoin has negative weights (min -10), so root must be lower.
...
def generate_build_nodes_query(motif_len):
    ...
    # FIXED: Use START_LEVEL instead of -1
    return f"""
...
"""
    for i, char in enumerate(motif_str):
        ...
        if layer < len(motif_str):
            ...
            # FIXED: Check against START_LEVEL
            q_switch = preamble + f"""
            MATCH (s_u:Stage {{accId: u.id, layer: {layer}}})
            WHERE s_u.level <> {START_LEVEL} AND {switch_pred} 
...
# FIXED: Use START_LEVEL
INIT_ITERATIVE_ROOTS = f"""
UNWIND $batchIds AS aid
MATCH (a:Account {{id: aid}})
MERGE (s:Stage {{accId: a.id, level: {START_LEVEL}, layer: 1}})
...
"""
        ...
        if layer < len(motif_str):
            ...
            next_layer = layer + 1
            # FIXED: Check against START_LEVEL
            pred_switch = f"u.layer = {layer} AND u.level <> {START_LEVEL} AND u.level {next_op} j"
...
def make_lifted_query_final(motif_len, max_hops_str):
    ...
    if is_iterative:
        ...
        return f"""
        CALL(){{
          MATCH (start:Stage {{level: {START_LEVEL}, layer: 1}})
...
# -----------------------------------------------------------------------------
# Data Loading (Bitcoin Alpha)
# -----------------------------------------------------------------------------

def download_and_parse_bitcoin(output_dir):
    print(f"[DEBUG] Downloading Bitcoin Alpha dataset from: {BITCOIN_ALPHA_URL}")
    try:
        response = requests.get(BITCOIN_ALPHA_URL, stream=True)
...
def init_results_file(filename):
    HEADERS = [
        ("Motif", 10), ("Hops", 5),
        ("B.Mean(ms)", 10), ("B.TO", 5),
        ("Build(ms)", 10), ("L.Mean(ms)", 10), ("L.TO", 5),
        ("Speedup", 15), ("Sanity", 8)
    ]
...
def append_result_row(filename, r):
    ...
    line = fmt.format(
        r["motif"], str(r["hops"]),
        f"{r['base_mean']:.2f}", r['base_timeouts'],
        f"{r['build_avg']:.2f}",
        f"{r['lift_mean']:.2f}", r['lift_timeouts'],
        s_str, r["sanity"]
    )
...
def main():
    global DEBUG
    parser = argparse.ArgumentParser(description="ZigZag Scaling (Bitcoin Alpha)")
    ...
    parser.add_argument("--out", default="results/zigzag_bitcoin")
    ...
    # Single Pass for Fixed Dataset
    print(f"\n=== PROCESSING BITCOIN ALPHA DATASET ===")
    
    # 1. Download & Prepare
    csv_path, node_count, edge_count = download_and_parse_bitcoin("/tmp/zigzag_btc_exp")
