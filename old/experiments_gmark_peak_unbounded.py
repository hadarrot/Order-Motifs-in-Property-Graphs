#!/usr/bin/env python3
"""
experiments_gmark_peak.py

- Base graph is generated with gMark.
- Motif: Peak (Strictly Increasing -> Strictly Decreasing).
- For each run: generate fresh graph, load, build StageLift (2 Layers), run queries.
- Writes TWO output files incrementally.
"""

# python3 experiments_gmark_peak_unbounded.py --password "ItayBachar88" --accounts 1000 --densities 1.0 5.0 10.0 --repeats 10 --timeout 60 --out results/peak_unbounded_results.csv
...
# ----------------------------
# Cypher fragments for Peak Motif (Up -> Down)
# ----------------------------
...
SCHEMA_STMTS = [
    ...
    # Updated Index to include 'layer'
...
# Build Nodes for Layer 1 (Up) and Layer 2 (Down)
# Layer 1 includes -1 (start). Layer 2 strictly needs history, so usually no -1, but for simplicity we gen all.
BUILD_STAGE_NODES = """
MATCH (v:Account)
...
MERGE (:Stage {accId: v.id, level: level, layer: 1})
MERGE (:Stage {accId: v.id, level: level, layer: 2})
"""

# Build Edges with 3 rules:
# 1. L1 -> L1 (Extend Up): level < j
# 2. L1 -> L2 (Switch Down): level > j (Switching condition is Down)
# 3. L2 -> L2 (Extend Down): level > j
BUILD_STAGE_EDGES = """
MATCH (u:Account)-[e:TRANSFER]->(v:Account)
WITH u, v, e, e.amount AS j, e.ts AS ts

// 1. Layer 1 Extension (Increasing)
FOREACH (ignore IN CASE WHEN true THEN [1] ELSE [] END |
  MERGE (su1:Stage {accId: u.id, level: -1, layer: 1}) // Start nodes
  MERGE (sv1:Stage {accId: v.id, level: j, layer: 1})
  MERGE (su1)-[r1:TRANSFER_LIFT {amount: j}]->(sv1)
  SET r1.ts = ts
)

WITH u, v, e, j, ts
MATCH (su:Stage {accId: u.id})
// Handle standard history nodes
WHERE su.level <> -1

// Case A: L1 -> L1 (Increasing)
FOREACH (ignore IN CASE WHEN su.layer = 1 AND su.level < j THEN [1] ELSE [] END |
  MERGE (sv:Stage {accId: v.id, level: j, layer: 1})
  MERGE (su)-[r:TRANSFER_LIFT {amount: j}]->(sv)
  SET r.ts = ts
)

// Case B: L1 -> L2 (Switching: Decreasing)
FOREACH (ignore IN CASE WHEN su.layer = 1 AND su.level > j THEN [1] ELSE [] END |
  MERGE (sv:Stage {accId: v.id, level: j, layer: 2})
  MERGE (su)-[r:TRANSFER_LIFT {amount: j}]->(sv)
  SET r.ts = ts
)

// Case C: L2 -> L2 (Decreasing)
FOREACH (ignore IN CASE WHEN su.layer = 2 AND su.level > j THEN [1] ELSE [] END |
  MERGE (sv:Stage {accId: v.id, level: j, layer: 2})
  MERGE (su)-[r:TRANSFER_LIFT {amount: j}]->(sv)
  SET r.ts = ts
)
...
# Baseline: Scan all paths and check for Peak property
# Peak: Exists split index 'k' such that path[0..k] is increasing AND path[k..end] is decreasing.
BASELINE_COUNT = """
CALL (){
  MATCH p = (s:Account)-[:TRANSFER*]->(t:Account)
  WITH s, p, [r IN relationships(p) | r.amount] AS amts, t
  WHERE size(amts) >= 2
  AND ANY(split_idx IN range(0, size(amts)-2) WHERE
      // 1. Increasing Part (0 to split_idx)
      ALL(i IN range(1, split_idx) WHERE amts[i-1] < amts[i])
      AND
      // 2. Switching Edge (split_idx to split_idx+1)
      amts[split_idx] > amts[split_idx+1]
      AND
      // 3. Decreasing Part (split_idx+1 to end)
      ALL(j IN range(split_idx+2, size(amts)-1) WHERE amts[j-1] > amts[j])
  )
  RETURN 1 AS row
}
...
# Lifted: Paths starting at L1 (start) and reaching any node in L2
LIFTED_COUNT = """
CALL (){
...
def main():
    ...
    ap = argparse.ArgumentParser(description="Baseline vs Lifted (gMark Peak Motif)")
    ...
    ap.add_argument("--out", default="results/experiment_peak.csv")
        ...
        for N in args.accounts:
            for D in args.densities:
                ...
                for r in tqdm(range(args.repeats), desc=desc_str):
                    output_dir = os.path.join(args.tmp_dir, f"gmark_peak_N{N}_E{E}_run{r}")
                    ...
                    with driver.session(database=args.database) as s:
                        ...
                        # Split edge building into chunks if needed, but here we run one block
                ...
                detail_row = {
                    ...
                    "baseline_avg_latency_ms": base_avg_details,
                    ...
                    "lift_avg_latency_ms": lift_avg_details,
                ...
                summary_row = {
                    ...
                    "Baseline Avg Latency (ms)": base_avg_summary,
                    "Lifted Avg Latency (ms)": lift_avg_summary,
                    "Speedup": speedup
                }
