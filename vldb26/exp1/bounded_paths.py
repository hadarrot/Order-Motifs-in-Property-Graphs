...
# python3 ./bounded_paths.py --password "ItayBachar88" --nodes 100 1000 10000 100000 1000000 100000
# -----------------------------------------------------------------------------
...
def clear_database(session):
    ...
    while True:
        ...
        if count == 0:
            break
        # Optional: Print dots or progress for massive deletions
...
def main():
    ...
    for N in args.nodes:
        ...
        for D in args.densities:
            ...
            for H in args.hops:
                print(f"  > Testing Hops={H}...")
                
                latencies = {"base": [], "lift": []}
                build_times = []
                timeouts = {"base": 0, "lift": 0}
                ...
                for r in range(args.repeats):
                    ...
                    try:
                        with driver.session(database=args.database) as session:
                            ...
                            build_times.append(lift_ms)

                        # Baseline
                        print(f"[DEBUG] Running Baseline Query (Hops={H})...")
                        try:
                            _, t_sec = run_timed_query(args.uri, auth, make_baseline_query(H), args.timeout, args.database)
                            latencies["base"].append(t_sec * 1000)
                            print(f"[DEBUG] Baseline Result: {t_sec*1000:.2f}ms")
                        except TimeoutError:
                            print("[DEBUG] Baseline Timed Out")
                            timeouts["base"] += 1
                        except Exception as e:
                            print(f"[DEBUG] Baseline Error: {e}")

                        # Lifted
                        print(f"[DEBUG] Running Lifted Query (Hops={H})...")
                        try:
                            _, t_sec = run_timed_query(args.uri, auth, make_lifted_query(H), args.timeout, args.database)
                            latencies["lift"].append(t_sec * 1000)
                            print(f"[DEBUG] Lifted Result: {t_sec*1000:.2f}ms")
                        except TimeoutError:
                            print("[DEBUG] Lifted Timed Out")
                            timeouts["lift"] += 1
                        except Exception as e:
                            print(f"[DEBUG] Lifted Error: {e}")
                ...
                def get_stats(data):
                    ...
                    return avg_val, mean_val, std_val

                _, b_mean, b_std = get_stats(latencies["base"])
                _, l_mean, l_std = get_stats(latencies["lift"])
                build_avg = sum(build_times) / len(build_times) if build_times else 0.0
                denominator = build_avg + l_mean
                speedup = (b_mean / denominator) if denominator > 0 else 0.0

                row = {
                    "nodes": N, "density": D, "edges": E, "hops": H,
                    "base_mean": b_mean, "base_std": b_std, "base_timeouts": timeouts["base"],
                    "build_avg": build_avg,
                    "lift_mean": l_mean, "lift_std": l_std, "lift_timeouts": timeouts["lift"],
                    "speedup": speedup
                }
                append_result_row(final_output_file, row)
                print(f"    Result: Base={b_mean:.2f}ms | Build={build_avg:.2f}ms | Lift={l_mean:.2f}ms | Speedup={speedup:.2f}x")
